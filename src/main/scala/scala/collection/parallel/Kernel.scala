...
object Kernel {
  ...
  import WorkstealingScheduler.{Ptr, Node, RangeNode}

  abstract class Range[T] extends Kernel[RangeNode[T], T] {
    def newRoot: Ptr[RangeNode[T], T] = {
      val work = new RangeNode[T](null, null)(0, size, RangeNode.range(0, size), WorkstealingScheduler.initialStep)
    ...
    def workOn(ws: WorkstealingScheduler)(tree: Ptr[RangeNode[T], T], size: Int): Boolean = {
      ...
      while (looping) {
        ...
        val p = RangeNode.progress(currrange)
        val u = RangeNode.until(currrange)
  
        if (!RangeNode.stolen(currrange) && !RangeNode.completed(currrange)) {
          if (rand.nextBoolean) {
            val newp = math.min(u, p + currstep)
            val newrange = RangeNode.range(newp, u)
            ...
            val newrange = RangeNode.range(p, newu)
      ...
      ws.completeNode(lsum, rsum, tree, this)
    }
  }
