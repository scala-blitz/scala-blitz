...
trait Workstealing[T] {
  ...
  abstract class Node[@specialized S, R](val left: Ptr[S, R], val right: Ptr[S, R])(@volatile var step: Int) {
    ...
    final def CAS_OWNER(ov: Owner, nv: Owner) = unsafe.compareAndSwapObject(this, OWNER_OFFSET, ov, nv)
    final def CAS_RESULT(ov: Option[R], nv: Option[R]) = unsafe.compareAndSwapObject(this, RESULT_OFFSET, ov, nv)
  ...
  final class Ptr[S, R](val up: Ptr[S, R], val level: Int)(@volatile var child: Node[S, R]) {
    def casChild(ov: Node[S, R], nv: Node[S, R]) = unsafe.compareAndSwapObject(this, CHILD_OFFSET, ov, nv)
    def writeChild(nv: Node[S, R]) = unsafe.putObjectVolatile(this, CHILD_OFFSET, nv)
...
object Workstealing {
  ...
  val OWNER_OFFSET = unsafe.objectFieldOffset(classOf[Workstealing[_]#Node[_, _]].getDeclaredField("owner"))
  val RESULT_OFFSET = unsafe.objectFieldOffset(classOf[Workstealing[_]#Node[_, _]].getDeclaredField("result"))
  val CHILD_OFFSET = unsafe.objectFieldOffset(classOf[Workstealing[_]#Ptr[_, _]].getDeclaredField("child"))
