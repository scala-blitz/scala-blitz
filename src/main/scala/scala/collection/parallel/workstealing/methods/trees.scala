...
object HashTrieSetMacros {
  ...
  def aggregate[T: c.WeakTypeTag, S: c.WeakTypeTag](c: Context)(z: c.Expr[S])(combop: c.Expr[(S, S) => S])(seqop: c.Expr[(S, T) => S])(ctx: c.Expr[WorkstealingTreeScheduler]): c.Expr[S] = {
    ...
    val result = reify {
      ...
      val kernel = new scala.collection.parallel.workstealing.Trees.HashSetKernel[T, S] {
        ...
        zv.splice
        def zero = zg.splice
        def combine(a: S, b: S) = { val r = comboper.splice.apply(a, b); println(" combining " + a + " " + b + " got " + r); r }
        def apply(node: Node[T, S], ci: Trees.HashSetIndexedStealer[T]): S = {
          var sum: S = zero
          println("Started with " + sum)
          println("1")
          while (ci.hasNext) {
            println("2")
            val el = ci.next
  ...
  def find[T: c.WeakTypeTag, U >: T: c.WeakTypeTag](c: Context)(pred: c.Expr[U => Boolean])(ctx: c.Expr[WorkstealingTreeScheduler]): c.Expr[Option[T]] = {
    ...
    val result = reify {
      ...
      val kernel = new scala.collection.parallel.workstealing.Trees.HashSetKernel[T, Option[T]] {
        ...
        def apply(node: Node[T, Option[T]], ci: Trees.HashSetIndexedStealer[T]): Option[T] = {
          var res: Option[T] = zero
          while (res.isEmpty && ci.hasNext) {
            val el = ci.next
  ...
  def mapReduce[T: c.WeakTypeTag, S >: T: c.WeakTypeTag, M: c.WeakTypeTag](c: Context)(mp: c.Expr[S => M])(combop: c.Expr[(M, M) => M])(ctx: c.Expr[WorkstealingTreeScheduler]): c.Expr[M] = {
    ...
    val result = reify {
      ...
      val kernel = new scala.collection.parallel.workstealing.Trees.HashSetKernel[T, ResultCell[M]] {
        ...
        def apply(node: Node[T, ResultCell[M]], ci: Trees.HashSetIndexedStealer[T]): ResultCell[M] = {
  ...
  def transformerKernel[T: c.WeakTypeTag, S: c.WeakTypeTag, That: c.WeakTypeTag](c: Context)(callee: c.Expr[Trees.HashSetOps[T]], mergerExpr: c.Expr[Merger[S, That]], applyer: c.Expr[(Merger[S, That], T) => Any]): c.Expr[Trees.HashSetKernel[T, Merger[S, That]]] = {
    ...
    reify {
      ...
      new Trees.HashSetKernel[T, Merger[S, That]] {
        ...
        def apply(node: Node[T, Merger[S, That]], ci: Trees.HashSetIndexedStealer[T]): Merger[S, That] = {
  ...
  def flatMap[T: c.WeakTypeTag, S: c.WeakTypeTag, That: c.WeakTypeTag](c: Context)(func: c.Expr[T => TraversableOnce[S]])(cmf: c.Expr[CanMergeFrom[Par[HashSet[T]], S, That]], ctx: c.Expr[WorkstealingTreeScheduler]): c.Expr[That] = {
    import c.universe._
  ...
  def filter[T: c.WeakTypeTag, That: c.WeakTypeTag](c: Context)(pred: c.Expr[T => Boolean])(cmf: c.Expr[CanMergeFrom[Par[HashSet[T]], T, That]], ctx: c.Expr[WorkstealingTreeScheduler]): c.Expr[That] = {
    import c.universe._
