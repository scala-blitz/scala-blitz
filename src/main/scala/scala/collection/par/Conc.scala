...
object Conc {
  ...
  trait BufferLike[T, +To, Repr <: BufferLike[T, To, Repr]] extends MergerLike[T, To, Repr] {
    ...
    private[par] final def pack() {
      if (lastSize > 0) conc = Append.apply(conc, new Chunk(lastChunk, lastSize))
    }

    private[par] final def prepareForMerge() = this.synchronized {
      if (lastSize > 0) {
  ...
  class ConcMerger[@specialized(Int, Long, Float, Double) T: ClassTag](
    ...
    private[par] var lastChunk: Array[T],
    private[par] var lastSize: Int
  ) extends BufferLike[T, Par[Conc[T]], ConcMerger[T]] with scala.collection.par.Merger[T, Par[Conc[T]]] {
    def classTag = implicitly[ClassTag[T]]
