...
class ParArrayBench extends OnlineRegressionReport with Serializable with ParArraySnippets with Generators {
  ...
  @transient
  lazy val customTasks = new scala.collection.parallel.ForkJoinTaskSupport(new scala.concurrent.forkjoin.ForkJoinPool(8))
...
//     }
    ...
    measure method "reduce" in {
      ...
      performance of "old" config (oldopts) in {
        using(arrays(small).map(_.to[collection.mutable.HashSet])) curve ("ParArray") in { a =>
          val pa = a.par
          pa.tasksupport = customTasks
          pa.reduce(_ + _)
        }
      }
    }
