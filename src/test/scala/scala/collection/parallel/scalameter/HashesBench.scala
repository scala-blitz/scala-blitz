...
class HashesBench extends PerformanceTest.Regression with Serializable with Generators {
  import Par._
  ...
  def hashMapMergerAdd(sz: Int) {
    val merger = new workstealing.Hashes.HashMapSimpleMerger[Int, Int](5, 450, 27, null)
    var i = 0
    while (i < sz) {
      val kv = (i, i)
      merger += kv
